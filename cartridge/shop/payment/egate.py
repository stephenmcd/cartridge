...
GATEWAY_URL = getattr(
    settings, "EGATE_GATEWAY_URL", "https://migs.mastercard.com.au/vpcdps"
)
...
try:
    ...
    raise ImproperlyConfigured(
        "You need to define EGATE_ACCESS_CODE and "
...
def process(request, order_form, order):
    ...
    post_data = {
        ...
        "vpc_CardExp": (
            request.POST["card_expiry_year"][2:].strip()
    ...
    try:
        ...
        if response["vpc_TxnResponseCode"] != "0":
            raise CheckoutError(_("Transaction declined: ") + response["vpc_Message"])
        else:
