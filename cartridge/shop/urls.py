...
urlpatterns = [
    re_path(r"^product/(?P<slug>.*)%s$" % _slash, views.product,
        name="shop_product"),
    ...
    path("checkout/complete%s" % _slash, views.complete,
        name="shop_complete"),
    re_path(r"^invoice/(?P<order_id>\d+)%s$" % _slash, views.invoice,
        name="shop_invoice"),
    re_path(r"^invoice/(?P<order_id>\d+)/resend%s$" % _slash,
